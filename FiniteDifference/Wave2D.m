...
nFrames = 425;                % number of frames in the movie
...
waveplot = surf(x,y,uCur(in,jn)); %, xlabel('X axis'), ylabel('Y axis'),
axis([0 xRight 0 yTop -.1 .1]);      %title('My title'), axis([0 xRight -1 1])
shading interp 
% colormap('bone')
...
for k = 2 : nFrames
   ...
   uFut(in,jn) = ...
    ...
    axis([0 xRight 0 yTop -.1 .1])  
    shading interp   %xlabel('X axis'), ylabel('Y axis'),
    %colormap('bone')  % title('My title'), axis([0 xRight -1 1])
